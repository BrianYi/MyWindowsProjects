...
void Show(HWND hwnd, HDC hdc, int xText, int yText, int iMapMode, TCHAR *szMapMode)
{
    ...
    RestoreDC(hdc, -1);
    TextOut(hdc, xText, yText, szBuffer, wsprintf(szBuffer, TEXT("%-20s%-8d%-12d%-8d%d"), 
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    PAINTSTRUCT ps;
    TEXTMETRIC  tm;
    static TCHAR    title[]     = TEXT("Mapping Mode        Left    Right       Top     Bottom");
    static TCHAR    undLine[]   = TEXT("------------        ----    -----       ---     ------");
    switch (message)
    {
    case WM_CREATE:
        hdc = GetDC(hwnd);
    ...
    case WM_PAINT:
        ...
        TextOut(hdc, 1, 1, title, lstrlen(title));
        TextOut(hdc, 1, 2, undLine, lstrlen(undLine));
