                           ...
                           WPARAM wParam, LPARAM lParam)
{
	 ...
	 case WM_MOUSEWHEEL:
		 iAccumDelta += ( short ) HIWORD( wParam );	// 这里一定要用(short),不然数值会出错
		 while ( iAccumDelta >= iDeltaPerLine )
		 {
			 SendMessage( hDlg, WM_VSCROLL, SB_LINEUP, ( LPARAM ) GetFocus( ) );
			 iAccumDelta -= iDeltaPerLine;
		 }
		 while ( iAccumDelta <= -iDeltaPerLine )
		 {
			 SendMessage( hDlg, WM_VSCROLL, SB_LINEDOWN, ( LPARAM ) GetFocus( ) );
			 iAccumDelta += iDeltaPerLine;
		 }
		 return TRUE;
